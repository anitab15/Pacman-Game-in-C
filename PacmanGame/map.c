...
void createmap(SDL_Surface* screen,char comp[20][12]) //plots the map from database
{
    ...
    border_pic=load_image("bods.gif");
...
SDL_Surface* build_cur;
build_cur=load_image("bc.gif");
SDL_Surface* build_follow;
build_follow=load_image("bcf.gif");
            ...
            switch (what_key)
                {
                  ...
                  SDL_BlitSurface(build_cur,NULL,screen,&plotter);

                   SDL_Flip(screen);
                    SDL_Delay(50);
                 SDL_BlitSurface(build_follow,NULL,screen,&plotter);
