    ...
    for (i=0;i<20;i++)
     {
        ...
        {
          //  ch='#';
           if ((comp[i][j]=='a' || comp[i][j]=='b' || comp[i][j]=='c' || comp[i][j]=='d' || comp[i][j]=='e' || comp[i][j]=='f' )) ch=comp[i][j];
...
void load_array(char comp[20][12],SDL_Surface* screen)
{
    ...
    SDL_Surface* border_pic=load_image("bods.gif");
